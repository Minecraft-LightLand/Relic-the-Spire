...
public class ApplyFragileEffect extends InstantEffect {

	public ApplyFragileEffect(MobEffectCategory category, int color) {
	...
	public void applyInstantenousEffect(@Nullable Entity self, @Nullable Entity owner, LivingEntity target, int amp, double dist) {
		if (dist == 1) {
			var old = target.getEffect(RtSEffect.FRAGILE.get());
			if (old == null) target.addEffect(new MobEffectInstance(RtSEffect.FRAGILE.get(), 900, amp));
